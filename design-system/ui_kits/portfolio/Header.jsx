/* Portfolio — Header.jsx · sticky top bar */
function PfHeader({ active, onNav }) {
  const links = [
    { id: 'work', label: '프로젝트' },
    { id: 'about', label: '소개' },
  ...
  const mark = {
    width: 34, height: 34, borderRadius: 'var(--r-sm)', background: 'var(--ink)',
    color: 'var(--paper)', display: 'flex', alignItems: 'center', justifyContent: 'center',
    fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: 14, letterSpacing: '.02em',
  };
  const nav = { display: 'flex', gap: 4, alignItems: 'center' };
  return (
    <header style={wrap}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={mark}>YH</div>
      ...
      <nav style={nav}>
        ...
        <a href="#" onClick={e => e.preventDefault()}
          ...
          <i data-lucide="file-text" style={{ width: 15, height: 15 }}></i>이력서
        </a>
      </nav>
    </header>
  );
